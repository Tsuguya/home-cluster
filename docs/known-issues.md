...
**影響**: ホストの `/usr/local/sbin/iscsiadm` が消失し、Trident CSI が iSCSI ボリュームをマウントできなくなる。SeaweedFS の volume が `ContainerCreating` でスタックする。filer は PostgreSQL (shared-pg) に移行済みのため iSCSI 不要。
